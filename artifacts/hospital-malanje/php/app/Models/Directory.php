...
final class HospitalDirectory
{
    ...
    {
        return Database::connection()->query('SELECT * FROM departments ORDER BY active DESC, name')->fetchAll();
    }

    public static function doctors(): array
    {
        return Database::connection()->query('SELECT doc.*, d.name AS department_name FROM doctors doc JOIN departments d ON d.id = doc.department_id ORDER BY doc.active DESC, doc.name')->fetchAll();
    }

    public static function createDepartment(string $name, string $color): void
    {
        $stmt = Database::connection()->prepare('INSERT INTO departments (name, color, active) VALUES (?, ?, TRUE)');
        $stmt->execute([$name, $color ?: '#2b7a78']);
    }

    public static function updateDepartment(int $id, string $name, bool $active): void
    {
        $stmt = Database::connection()->prepare('UPDATE departments SET name = ?, active = ? WHERE id = ?');
    ...
    public static function createDoctor(string $name, string $specialty, int $departmentId): void
    {
        ...
        $stmt = Database::connection()->prepare('INSERT INTO doctors (name, specialty, department_id, initials, active) VALUES (?, ?, ?, ?, TRUE)');
    ...
    {
        $stmt = Database::connection()->prepare('UPDATE doctors SET name = ?, specialty = ?, department_id = ?, active = ? WHERE id = ?');

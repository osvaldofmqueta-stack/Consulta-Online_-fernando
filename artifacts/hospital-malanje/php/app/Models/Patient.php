...
final class Patient
{
    ...
    {
        $stmt = Database::connection()->prepare('SELECT * FROM pacientes WHERE UPPER(medical_record_number) = UPPER(?) AND phone = ?');
    ...
    {
        return Database::connection()->query('SELECT * FROM pacientes ORDER BY created_at DESC')->fetchAll();
    }

    public static function forDoctor(int $doctorId): array
    {
        $stmt = Database::connection()->prepare('SELECT DISTINCT p.* FROM pacientes p JOIN consultas a ON a.patient_id = p.id WHERE a.doctor_id = ? ORDER BY p.created_at DESC');
